...
const PlayXCoin = artifacts.require('PlayXCoin.sol');
const PlayXCoinMultiSigFactory = artifacts.require('PlayXCoinMultiSigWalletWithMint.sol');
const web3 = PlayXCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return PlayXCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};
...
contract("PlayXCoinMultiSigWalletWithMint", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OnePlayXCoinInDennis;
  let NoOfTokens, NoOfTokensInDennis;
  ...
  beforeEach(async () => {
    coinInstance = await deployPlayXCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInDennis = await coinInstance.totalSupply();
    OnePlayXCoinInDennis = await coinInstance.getOnePlayXCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.plxCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test 1", {from: user1}));
    assertRevert("1-2",() => walletInstance.requestTransferInDennis(user1, 1, "test 2", {from: user1}));
    ...
    assertRevert("1-8",() => walletInstance.requestMinterAdd(user2, "test 8", {from: user1}));

    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test 2-1", {from: owner}));
    assertRevert("2-2",() => walletInstance.requestTransferInDennis(user1, 1, "test 2-2", {from: owner}));
    ...
    assertRevert("2-8",() => walletInstance.requestMinterAdd(user2, "test 2-8", {from: owner}));

    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test 3-1", {from: admin}));
    assertNotRevert("3-2",() => walletInstance.requestTransferInDennis(user1, 1, "test 3-2", {from: admin}));
    ...
    assertNotRevert("3-8",() => walletInstance.requestMinterAdd(user2, "test 3-8", {from: admin}));

    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test 4-1", {from: vault}));
    assertNotRevert("4-2",() => walletInstance.requestTransferInDennis(user1, 1, "test 4-2", {from: vault}));
    ...
    assertNotRevert("3-8",() => walletInstance.requestMinterAdd(user2, "test 4-8", {from: vault}));

    // signer(as signer) can
    assertNotRevert("5-1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test 5-1", {from: signer}));
    assertNotRevert("5-2",() => walletInstance.requestTransferInDennis(user1, 1, "test 5-2", {from: signer}));
    ...
    assertNotRevert("5-8",() => walletInstance.requestMinterAdd(user2, "test 5-8", {from: signer}));
  });

  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInPLX(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    assert.equal(vaultInitBalance.toString(), NoOfTokensInDennis.toString());
    assert.equal((await ethBalanceOf(walletInstance.address)).toString(),"0");  // initially no eth on walletInstance(gas fee reserve)

    const thousandPlayXInDennis = OnePlayXCoinInDennis.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandPlayXInDennis, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandPlayXInDennis.toString());

    // Add plxcoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OnePlayXCoin * 1);
    //

    const MSG1 = "send 1PLX and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInPLX(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OnePlayXCoinInDennis.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test addsigner-1", {from: user1}));
    assertNotRevert("signer2",() => walletInstance.requestTransferInDennis(user1, 1, "test addsigner-2", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInPLX(user1, 1, 0, "test addsigner-1", {from: user1}));
    assertRevert("signer2",() => walletInstance.requestTransferInDennis(user1, 1, "test addsigner-2", {from: user1}));
  ...
  it("Add and remove a minter", async () => {
      ...
      assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OnePlayXCoinInDennis, {from: user1}));
      let afterBalance = await coinInstance.balanceOf(user1);

      // balance difference should be same as the minted amount
      assert.equal(afterBalance.sub(prevBalance).toString(), OnePlayXCoinInDennis.toString());
      ...
      assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OnePlayXCoinInDennis, {from: user1}));
  ...
  it("test mint using multisig wallet", async () => {
        ...
        const thousandPlayXInDennis = OnePlayXCoinInDennis.mul(BIG(1000))
        const thousandPlayX = BIG(1000)
        const zero = BIG(0)

        const MSG1 = "MINT 1000 PLX to user1";
        const trSubmit = await walletInstance.requestMintInPLX(user1, thousandPlayX, zero, MSG1, {from:vault});
        ...
        assert.equal(afterBalance.toString(), thousandPlayXInDennis.toString());
